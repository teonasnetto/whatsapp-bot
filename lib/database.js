...
module.exports = {
    ...
    verificarDonoAtual : async(id_usuario)=>{
        let {limite_diario} = JSON.parse(fs.readFileSync(path.resolve("database/json/bot.json")))
        let usuario = await db.usuarios.asyncFindOne({id_usuario, tipo: "dono"})
        if(!usuario){
    ...
    obterTodosGrupos: async()=>{
        let todos_grupos = await db.grupos.asyncFind({})
        return todos_grupos
    },
    //###
    ...
    participanteExiste: async (id_grupo, id_usuario)=>{
        let grupo = await db.grupos.asyncFindOne({id_grupo})
        return (grupo?.participantes.includes(id_usuario))
    },
    //###

    //### ALTERAR RECURSOS

    alterarGrupoAtivo: async(id_grupo, status = true)=>{
        await db.grupos.asyncUpdate({id_grupo}, {$set:{grupo_ativo: status}})
    },
