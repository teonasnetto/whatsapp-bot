...
module.exports = checagemMensagem = async (client, message) => {
    try {
        ...
        //SE FOR GRUPO E NÃO TIVER A INFO REGISTRADA no db (PRECISA SEMPRE ESTAR EM PRIMEIRO)
        if(isGroupMsg && !grupoInfo) return false

        //SE O GRUPO NÃO ESTIVER ATIVO
        if(isGroupMsg && !grupoInfo.grupo_ativo) return false
