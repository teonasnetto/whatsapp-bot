    ...
    try{
        ...
        switch(command){
            ...
            case '!ajuda':
                ...
                break
            case '!ping':
                var dataInicial = Date.now()
                await client.reply(chatId, "Pong!", id)
                var dataFinal = Date.now()
                var ping = dataFinal - dataInicial
                await client.reply(chatId, criarTexto(msgs_texto.info.ping.resposta, ping), id)
                break
        }
    } catch(err){
        throw err
